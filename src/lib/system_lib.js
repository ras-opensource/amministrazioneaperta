...
async function AA_UpdateFilterParams(reverse=false)
{
    ...
    if(!dlg.length)
    {
        ...
        //prepara la query dei parametri
        if(AA_MainApp.searchBoxParams.query=="")
        {
            AA_MainApp.searchBoxParams.query="stato_scheda_search=1";
            if(AA_MainApp.searchBoxParams.stato_scheda_search == 2)
            {
                AA_MainApp.searchBoxParams.query="stato_scheda_search=2";
            }
            if (AA_MainApp.searchBoxParams.stato_cestinata_search == 1) AA_MainApp.searchBoxParams.query+="&stato_cestinata_search=1";
            if (AA_MainApp.searchBoxParams.stato_revisionata_search == 1) AA_MainApp.searchBoxParams.query+="&stato_revisionata_search=1";

            if(AA_MainApp.searchBoxParams.goToPage > 0) AA_MainApp.searchBoxParams.query+="&goPage="+AA_MainApp.searchBoxParams.goToPage;

            AA_MainApp.searchBoxParams.query+="&id_assessorato_search="+AA_MainApp.searchBoxParams.id_assessorato_search;
            AA_MainApp.searchBoxParams.query+="&id_direzione_search="+AA_MainApp.searchBoxParams.id_direzione_search;
            AA_MainApp.searchBoxParams.query+="&id_servizio_search="+AA_MainApp.searchBoxParams.id_servizio_search;
    ...
    AA_MainApp.searchBoxParams.query = $("form#"+AA_MainApp.searchBoxParams.id+"_form", dlg).serialize();
    console.log(AA_MainApp);
...
async function AA_MainSearchDlg(params)
{
    ...
    if(searchBox.length > 0)
    {
        ...
        console.log("Rimuovo il box esistente...");
        searchBox.remove();
    } 
...
function AA_SetupMainUi()
{
        ...
        cols:[
            {
                ...
                view: "button", type: "icon", icon: "mdi mdi-menu",width: 44, align: "left", css: "AA_App_button", tooltip: "Mostra/nascondi la sidebar", click: function(){$$("AA_MainSidebar").toggle();}
            },
            ...
            { id: "AA_navbar_assistant", view: "layout", type: "clean", cols: [{},{view: "template", type:"clean", width: "130", template:"<div class='AA_navbar_link_box_right AA_AMAAI_btn'><a><span class='mdi mdi-compass'></span>Aiuto</a></div>", tooltip: "Avvia l'assistente digitale (AMAAI) per la navigazione assistita.", onClick: {"AA_AMAAI_btn":AA_MainApp.ui.MainUI.AMAAI.start}}]},
            {
                ...
                view: "button", type: "icon", icon: "mdi mdi-dots-vertical",width: 44, align: "left", css: "AA_App_button", tooltip: "Fai click per visualizzare le azioni disponibili per la sezione corrente.",
    ...
    webix.ui({
        ...
        rows: [
            {view: "layout", type: "clean", css: "AA_header", height: 60, 
            cols: [
            ...
            { 
                id: "AA_footer_box",
                view: "layout", css: "AA_footer_box",cols: [
                    {view: "spacer"}
                ],
                height: 12
            }
