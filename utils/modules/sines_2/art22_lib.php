    ...
    static public function Search($params,$bOnlyCount=false, $user=null)
    {
        ...
        if($params['in_corso'] !="" || $params['in_scadenza'] !="" || $params['scadute'] !="" || $params['recenti'] !="")
        {
            ...
            if($params['scadute'] =="0")
            {
                ...
                else $having.=" AND data_fine_incarico >= '".$mesePrec->format("Y-m-d")."' ";
            }
            
            //Scadono entro un mese
            if($params['in_scadenza'] == "0")
            {                                
                if($having =="") $having.=" HAVING (data_fine_incarico > '".$meseProx->format("Y-m-d")."' OR data_fine_incarico < '". addslashes($params['data_scadenzario'])."') ";
                else $having.=" AND (data_fine_incarico > '".$meseProx->format("Y-m-d")."' OR data_fine_incarico < '".addslashes ($params['data_scadenzario'])."') ";
            }

            //Scadute da meno di un mese
            if($params['recenti'] == "0")
            {
                if($having =="") $having.=" HAVING (data_fine_incarico < '".$mesePrec->format("Y-m-d")."' OR data_fine_incarico > '".addslashes($params['data_scadenzario'])."') ";
                else $having.=" AND (data_fine_incarico < '".$mesePrec->format("Y-m-d")."' OR data_fine_incarico > '".addslashes($params['data_scadenzario'])."') "; 
            }
...
Class AA_OrganismiReportNomineListTemplateView extends AA_GenericTableTemplateView
{
    public function __construct($id="AA_OrganismiReportNomineListTemplateView",$parent=null,$organismo=null, $user=null)
    {
        ...
        if($num_nomine>0)
        {
            ...
            if($num_nomine_ras >0) $footer.="<div style='font-style: italic; text-align: left; width: 100%; margin-top: .3em;font-size: smaller;'>Sono ".$num_nomine_ras." i componenti nominati dall'Amministrazione regionale negli organi di governo.</div>";
            else $footer.="<div style='font-style: italic; text-align: left; width: 100%; margin-top: .3em;font-size: smaller;'>Non sono presenti componenti nominati dall'Amministrazione regionale negli organi di governo.</div>";
